...
from utils.file_utils import sync_checkpoints, map_network_volume

# Map network volue if one is mounted
result, error = map_network_volume()
if error:
    print(f'[ToonCrafter][WARNING]: Could not map network volume: {error}')

# Initial load (if needed) to populate network volume with checkpoints
result, error = sync_checkpoints()

if error:
    print(f'[ToonCrafter][ERROR]: Failed to download checkpoints: {error}')
    sys.exit(1)
...
if __name__ == "__main__":
    ...
    dynamicrafter_iface.launch(server_name='0.0.0.0', server_port=7860, max_threads=1)
